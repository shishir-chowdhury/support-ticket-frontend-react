...
export default function TicketDetails({ token }) {
    ...
    const handleAddComment = async (e) => {
        ...
        try {
            const response = await addComment(id, messageText, token);
    ...
    useEffect(() => {
        ...
        console.log("Connecting to channel:", `ticket.${id}`);
        const channel = echoInstance.private(`ticket.${id}`);

        channel.listen("MessageSent", (e) => {
            console.log("MessageSent received:", e);
            console.log("Comment data:", e.comment);

            setComments((prev) => {
                const exists = prev.find(c => c.id === e.comment.id);
                if (exists) {
    ...
    if (!ticket)
        return <p className="text-center text-gray-500 mt-10">Ticket not found.</p>;

    return (
        <div className="max-w-6xl mx-auto bg-white shadow-lg rounded-xl p-8 mt-10">
            {/* Ticket Header */}
            ...
            <div className="mt-6">
                ...
                <div className="border rounded-lg bg-gray-50 p-4 mb-4 max-h-[450px] overflow-y-auto">
                    {comments.length === 0 && (
